...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", required=True, help="split for splitting video for saving mp4")
    parser.add_argument("--path_train", required=True, help="path to train set")
    parser.add_argument("--path_test", required=True, help="path to test set")
    ...
    if opt.mode == 'split':
        path = [opt.path_train, opt.path_test]
        split_videos(path)

    elif opt.mode == 'video':
        paths = [(opt.path_train, opt.path_train),
                ( opt.path_test, opt.path_test)]
        save_mp4(paths)
    else:
        raise ValueError("given mode should be either split or video")